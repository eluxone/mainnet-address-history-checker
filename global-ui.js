...
(()=>{
  ...
  const fallback=[
    ...
    {href:'/login.html',label:'Private Access',file:'login.html'}
  ];
  ...
  const iconFor=(label,file)=>{const t=`${label} ${file}`.toLowerCase();if(t.includes('private')||t.includes('login'))return'⌾';if(t.includes('btc')||t.includes('bitcoin'))return'₿';if(t.includes('recover'))return'↺';if(t.includes('approval'))return'✓';if(t.includes('wallet'))return'◇';if(t.includes('address'))return'⌁';if(t.includes('research'))return'◫';return'Ξ'};
  const descFor=(label,file)=>{const t=`${label} ${file}`.toLowerCase();if(t.includes('private')||t.includes('login'))return'Protected deployment access';if(t.includes('btc')||t.includes('bitcoin'))return'Bitcoin public-data research';if(t.includes('recover'))return'Owner-authorized recovery tools';if(t.includes('approval'))return'Public token approval research';if(t.includes('wallet'))return'Wallet and address utilities';if(t.includes('address'))return'Public address analysis';return'Crypto research tool'};
  const escapeHtml=value=>String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

  const header=document.createElement('header');
  header.className='crypto-global-nav';
  header.innerHTML=`<div class="crypto-nav-inner"><a class="crypto-brand" href="/" aria-label="Crypto research home"><span class="crypto-brand-mark">◈</span><span class="crypto-brand-copy">CHAIN//LAB<small>public crypto intelligence</small></span></a><nav class="crypto-desktop-links" aria-label="Primary navigation"></nav><div class="crypto-nav-actions"><button class="crypto-tools-button" type="button" aria-expanded="false">All tools <span aria-hidden="true">⌄</span></button><button class="crypto-mobile-toggle" type="button" aria-label="Open navigation" aria-expanded="false"><span></span></button></div></div><div class="crypto-tools-menu" role="menu"></div>`;
  document.body.prepend(header);
  const desktop=header.querySelector('.crypto-desktop-links');
  for(const item of items.slice(0,5)){const a=document.createElement('a');a.className=`crypto-nav-link${active(item.href)?' active':''}`;a.href=item.href;a.textContent=item.shortLabel||item.label;desktop.append(a)}
