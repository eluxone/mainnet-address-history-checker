...
function parseServiceAccount() {
  ...
  if (packed) {
    for (const candidate of [packed, (() => { try { return Buffer.from(packed, 'base64').toString('utf8'); } catch { return ''; } })()]) {
      try {
...
function buildCandidateQuery(filters) {
  // Date strings are already strict YYYY-MM-DD values and numeric values are validated.
...
async function fetchJsonRetry(url, attempts = 3) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const response = await fetchTimeout(url, { headers: { Accept: 'application/json' } }, 15_000);
...
async function checkAddress(candidate) {
  ...
  const [summary, txs] = await Promise.all([
    fetchJsonRetry(`${base}/address/${address}`),
    fetchJsonRetry(`${base}/address/${address}/txs`)
  ]);
...
export default async function handler(request, response) {
  ...
  if (expected && !safeTokenEqual(request.headers['x-app-access-token'], expected)) return send(response, 401, { error: 'Incorrect or missing app passcode.' });
  ...
  try {
    ...
    return send(response, status, { error: error?.name === 'AbortError' ? 'A public-data provider timed out.' : error?.message || 'Unable to search public Bitcoin data.' });
  }
}
